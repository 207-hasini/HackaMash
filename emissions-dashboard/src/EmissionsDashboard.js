...
function EmissionsDashboard() {
  ...
  const totalKwh = 14250; // Simulated static value
  const activeAlerts = stores.filter(s => s.status === 'critical' || s.status === 'warning').length;

  // Store selection handler
  const handleStoreClick = (storeId) => {
    const storeToDisplay = stores.find(s => s.id === storeId);
    setShowStoreDetail(storeToDisplay);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100 p-4">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-blue-400 mb-2 md:mb-0">Dynamic Emissions Optimizer</h1>
        <div className="text-lg font-mono text-gray-300">{currentTime.toLocaleString()}</div>
      </div>
      {/* Filters */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KpiCard
          ...
          hoverBgColor="hover:bg-gray-800/70"
        />
        <KpiCard
          icon={Target}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6">
          <h3 className="text-xl font-semibold mb-4 flex items-center">Store Status Overview</h3>
      ...
      {/* Store Detail Modal */}
      {showStoreDetail && (
        <StoreDetailView store={showStoreDetail} onClose={() => setShowStoreDetail(null)} />
      )}
    </div>
  );
}
